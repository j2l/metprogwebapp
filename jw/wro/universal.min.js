...
function loadInboxData(){$(".inbox-notification .loading").show();var url=$(".inbox-notification").data("url");$.getJSON(url+"&_t="+(new Date()),{},function(data){var count=0;if(data.count!==undefined){count=data.count;}
...
console.log('Service worker successfully registered and activated.');navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({userviewKey:PwaUtil.userviewKey,homePageLink:PwaUtil.homePageLink});}
...
$offlineIndicator.show();},isReachable:function(url){return fetch(url,{method:'HEAD',mode:'no-cors'}).then(function(resp){return resp&&(resp.ok||resp.type==='opaque');}).catch(function(err){console.warn('isReachable failed '+url,err);return false;});},handleConnection:function(){}}
